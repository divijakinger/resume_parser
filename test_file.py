...
text = ""
import os
file_path = 'D:\Project_Deep_Blue_Archive\Archive\Test_pdfs'
output_path = 'D:\Project_Deep_Blue_Archive\Archive\output'

# for files in os.listdir(file_path):
#     if files.endswith(".docx"):
#         docx2pdf.convert(files, output_path)
#     elif files.endswith(".html"):
#         config = pdfkit.configuration(wkhtmltopdf=r"C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe")
#         kitoptions = {
#             "enable-local-file-access": None
#         }
#         pdfkit.from_file(files, output_path, configuration=config, options=kitoptions)
#
#     # convert rtf to pdf
#     elif files.endswith(".rtf"):
#         change_word_format(files)
#
#     # convert doc to pdf
#     elif files.endswith(".doc"):
#         doc2pdf.convert(files, output_path)
#     else:
#         continue
...
def extract_name_possibility_1(resume_text):
    ...
    prefix = ['Dr.', 'DR.', 'MD.', 'MD', 'Md.', 'Md', 'Mr.', 'MR.', 'MR', 'Mr']
    sents = en(resume_text)
    # people = [ee for ee in sents.ents if ee.label_ == 'PERSON']
    people = []
    for ee in sents.ents:
        if ee.label_ == 'PERSON':
            if ee.start != 0:
                prev_token = sents[ee.start - 1]
                if prev_token.text in prefix:
                    new_people = Span(sents, ee.start - 1, ee.end, label=ee.label)
...
def extract_name_possibility_2(resume_text):
    ...
    matcher.add('NAME', None, [*pattern_2])
...
for name in dir_list:
    ...
    doc = nlp(text)
    noun_chunks = list(doc.noun_chunks)
    name_test_1.append(extract_name_possibility_1(text))
    # print("Name possibility 1: ", extract_name_possibility_1(text))
    name_test_2.append(extract_name_possibility_2(text))
...
with open('Output_entity.txt', 'w') as f:
    for item in name_test_1:
        f.write("%s\n" % item)

with open('Output_traditional.txt', 'w') as f:
    for item in name_test_2:
        f.write("%s\n" % item)
